...
class Zavudev::Test::SingletonClient < Zavudev::Client
  include Singleton

  TEST_API_BASE_URL = ENV.fetch("TEST_API_BASE_URL", "http://localhost:4010")

  def initialize
    super(base_url: Zavudev::Test::SingletonClient::TEST_API_BASE_URL, api_key: "My API Key")
  end
end
...
class Zavudev::Test::ResourceTest < Minitest::Test
  def async?
  ...
  def before_all
    super
    @zavudev = Zavudev::Test::SingletonClient.instance
  end
